...
const char * const BITCOIN_CONF_FILENAME = "citrus.conf";
const char * const BITCOIN_PID_FILENAME = "citrusd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "citrus" is a composite category enabling all Citrus-related debug output
            if(ptrCategory->count(string("citrus"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CitrusCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CitrusCore
    // Mac: ~/Library/Application Support/CitrusCore
    // Unix: ~/.citruscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CitrusCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CitrusCore";
#else
    // Unix
    return pathRet / ".citruscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty citrus.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override citrus.conf
